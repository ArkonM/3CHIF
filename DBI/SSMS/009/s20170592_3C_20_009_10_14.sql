...
, seit   date        not null
, titel  varchar(6)           -- NULL bedeutet kein Titel
, check (gebdat < seit)
);
go
...
, check (knr <> knrvor)   -- Kurs kann sich nicht selbst als Voraussetzung haben
);
go
...
, pnr     integer              references referent -- NULL wenn kein Referent
...
, foreign key (knr, knrlfnd) references kveranst -- Zusammengesetzter FK, da PK
);                                               -- in kveranst zusammengesetzt
go
...
, (  4,       1, '12.01.2004', '13.01.2004', 'Wien',          3,  116)
...
, (  7,       2, '14.09.2005', '18.09.2005', 'Muenchen',      4,  116)
;
...
-- Alle Fremdschlüssel indizieren, um Problemen mit Sperren vorzubeugen.
...
create index idx_kveranst_pnr
    on kveranst(pnr)
;
go

create index idx_besucht_knr_knrlfnd
    on besucht(knr, knrlfnd)
;
go
